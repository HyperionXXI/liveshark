...
\subsection{Schéma minimal (champs et types)}
\begin{itemize}
  \item \texttt{report\_version} : entier (version de schéma).
  \item \texttt{tool} : objet \{\texttt{name} : chaîne, \texttt{version} : chaîne\}.
  \item \texttt{generated\_at} : chaîne, horodatage RFC3339.
  \item \texttt{input} : objet \{\texttt{path} : chaîne, \texttt{bytes} : entier\}.
  \item \texttt{capture\_summary} : objet ou null (si indisponible).
  \item \texttt{universes[]}, \texttt{flows[]}, \texttt{conflicts[]}, \texttt{compliance[]} : tableaux (peuvent être vides en v0.1).
\end{itemize}

\subsection{Règles de déterminisme}
\begin{itemize}
...
\subsection{Définition canonique d'une trame DMX}
...
\subsection{Règles minimales de reconstruction}
\begin{itemize}
...
\section{Appendice C --- Contrat de détection de conflits (v0.1)}
\subsection{Identité de source}
\begin{itemize}
...
\subsection{Définition du chevauchement}
