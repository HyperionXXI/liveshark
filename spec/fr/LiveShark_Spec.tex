...
LiveShark est développé en \textbf{Rust} pour concilier \textbf{sécurité mémoire} et \textbf{performances}.
Il privilégie aussi la \textbf{portabilité} (Windows, macOS, Linux) et un outillage moderne. Il s'agit d'un choix
d'ingénierie pour v0.1 et cela n'exclut pas l'usage d'autres langages pour des composants futurs si cela se justifie.
...
\begin{itemize}
  ...
  et \texttt{violations[]} (tableau d'objets avec : \texttt{id} (identifiant stable), \texttt{severity} (chaîne ; v0.1 utilise \texttt{warning} ou \texttt{error}),
  \texttt{message} (explication lisible), \texttt{count} (entier, nombre d'occurrences), et \texttt{examples[]} optionnel
  (tableau d'au plus 3 chaînes, chacune donnant un contexte concis tel que \texttt{"source IP:port @ timestamp"}).
  S'ils sont présents, les exemples \DOIT{} être dédupliqués, triés de façon stable et limités à 3 pour garder des rapports compacts et déterministes).
\end{itemize}
...
\texttt{universe\_id = 256 * net + subuni} (0..32767). Pour sACN, c'est le champ \texttt{universe} en big-endian de la couche
