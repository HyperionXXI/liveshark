...
\begin{itemize}
  ...
  \item \textbf{RTP (générique) (Messages) :} trous de séquence, gigue, désordre ; base pour bilans de santé de type AES67.
  \item \textbf{RTP-MIDI (Messages) :} timing des messages MIDI ; corrélation avec signaux de conduite/codes temporels sur le réseau.
  \item \textbf{Infrastructure réseau (Flux) :} motifs IGMP/mDNS/DHCP/ARP/DNS ; problèmes de multicast/découverte/adressage.
  \item \textbf{Protocoles laser propriétaires (Flux) :} perte/gigue/rafales par flux ; détection multi-sources si applicable.
\end{itemize}
...
\begin{itemize}
  \item \textbf{pps/bps :} paquets/octets par seconde sur une fenêtre glissante de 1.0 s.
  \item \textbf{fps (univers) :} trames DMX reconstruites par seconde sur une fenêtre glissante de 5.0 s.
  \item \textbf{gigue (IAT) :} variation moyenne entre temps d'arrivée consécutifs sur une fenêtre glissante de 10.0 s (heuristique, sans interpolation).
  \item \textbf{perte (observée) :} lorsque des numéros de séquence existent (ex. sACN), la perte est rapportée comme des trous observés au point de capture ; sinon, elle est estimée par une heuristique fondée sur les ruptures de cadence. Cela ne prouve pas une perte côté récepteur.
\end{itemize}
...
Le rapport JSON \DOIT{} inclure les champs minimaux définis dans l'Appendice A (v0.1).
\end{reqbox}

\begin{reqbox}{LS-REP-011}{P1}{}
Le rapport JSON \DOIT{} respecter les règles de déterminisme définies dans l'Appendice A (tri stable et champs volatils).
\end{reqbox}

\begin{reqbox}{LS-REP-012}{P1}{}
Le rapport JSON \DOIT{} inclure les exemples décrits dans l'Appendice A (minimal + conflit).
\end{reqbox}
...
LiveShark \DOIT{} calculer et rapporter pps/bps/fps/gigue tels que définis dans la Section 4.1.
\end{reqbox}
...
\begin{itemize}
  ...
  \item \textbf{Détection des conflits} : basée uniquement sur le chevauchement temporel, avec des faux positifs possibles si les sources agissent sur des canaux différents.
...
\begin{enumerate}
  \item \texttt{liveshark pcap analyse --report out/report.json file.pcapng} fonctionne sur 3 captures.
...
\begin{itemize}
  ...
  \texttt{sources[]} (tableau d'objets avec \texttt{source\_ip}, \texttt{cid} optionnel et \texttt{source\_name} optionnel ;
  ...
  \item Les éléments de \texttt{compliance[]} contiennent : \texttt{protocol} (chaîne), \texttt{compliance\_percentage} (flottant ; v0.1 utilise 100.0 comme valeur de remplacement),
  et \texttt{violations[]} (tableau d'objets avec \texttt{id}, \texttt{severity}, \texttt{message}, \texttt{count}, et \texttt{examples[]} optionnel,
  limité à 3 chaînes).
\end{itemize}
...
{
  "universe": 1,
  "sources": ["sacn:cid:11223344556677889900aabbccddeeff", "artnet:192.168.0.2:6454"],
...
\begin{itemize}
  ...
  \item Avant la première trame observée pour une source et un univers, les valeurs de slots sont à 0 par défaut.
  \item Si une charge utile fournit moins de 512 slots, les slots manquants conservent la dernière valeur connue pour cette source et cet univers.
...
L'univers canonique est l'entier 16 bits décodé depuis la charge utile du protocole. Pour Art-Net, c'est le champ
\texttt{universe} en little-endian d'ArtDMX, avec \texttt{universe\_id = 256 * net + subuni} (0..32767). Pour sACN, c'est le
champ \texttt{universe} en big-endian de la couche de framing. Le rapport utilise cette valeur canonique ; un même univers
signifie des valeurs numériques identiques entre protocoles.
...
En v0.1, la détection de conflit est une approximation prudente basée sur le chevauchement temporel ; des faux positifs
sont possibles lorsque les sources ne concurrencent pas activement les mêmes canaux au même moment.
...
\begin{itemize}
  \item \textbf{pps/bps :} paquets/octets par seconde sur une fenêtre glissante de 1.0 s ; unités paquets/s et octets/s.
  \item \textbf{fps (univers) :} trames DMX reconstruites par seconde sur une fenêtre glissante de 5.0 s ; unité trames/s.
  \item \textbf{gigue (IAT) :} variation moyenne entre temps d'arrivée consécutifs sur une fenêtre glissante de 10.0 s ; unité en secondes (rapportée en ms si applicable).
  \item \textbf{perte (observée) :} pour les sources avec numéros de séquence, la perte est la somme des trous de séquence observés dans la fenêtre glissante ;
  \texttt{loss\_rate} est \texttt{loss\_packets / (loss\_packets + frames\_count)} pour les trames suivies par séquence.
  \item \textbf{rafales :} \texttt{burst\_count} est le nombre de rafales de pertes dans la fenêtre glissante ; \texttt{max\_burst\_len} est la
