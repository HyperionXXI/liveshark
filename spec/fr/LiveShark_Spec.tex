...
\begin{tcolorbox}
\textbf{Périmètre.} LiveShark est un analyseur \textbf{passif} pour les réseaux de show-control (Art-Net, sACN).
...
L'interface idéale pour l'utilisateur final est une \textbf{GUI} bien conçue ; la \textbf{CLI} reste scriptable et produit les mêmes données.
\end{tcolorbox}
...
Dans le nom du projet, \enquote{Live} fait référence au \textbf{spectacle vivant / show control} (réseaux lumière), et non à une
...
\begin{itemize}
  ...
  \item Rapports JSON versionnés et reproductibles pour tickets, post-mortem, QA et CI.
\end{itemize}
...
\subsection{Stratégie \emph{offline-first} (intention produit)}
LiveShark est conçu comme un analyseur \emph{offline-first} : les premières versions se concentrent sur l'analyse post-mortem
...
Le mode suivi constitue une trajectoire pragmatique pour un diagnostic fiable pendant la préparation du show et pendant le show :
...
\begin{itemize}
  ...
  \item \textbf{Fenêtre jitter :} 10.0 s (glissante) (référence conceptuelle : RFC 3550)
\end{itemize}
