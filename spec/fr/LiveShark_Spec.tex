...
\end{itemize}
Voir les Appendices A--D pour les contrats normatifs (rapport, trames, conflits, m?triques).
...
LiveShark est conçu pour le diagnostic. Les extensions DOIVENT améliorer un diagnostic exploitable (perte, gigue, rafales, conflits, multi-sources) et DOIVENT rester reproductibles (mêmes entrées $\rightarrow$ mêmes sorties).
...
LiveShark PEUT fournir des \emph{pistes de causes probables} uniquement sous forme d’heuristiques. Ces pistes DOIVENT être justifiées par des motifs observables et NE DOIVENT PAS être présentées comme des causes certaines.
...
\toprule
\textbf{Extension (candidate)} & \textbf{Niveau} & \textbf{Valeur diagnostic (exemples)} \\
\midrule
OSC & Messages & Chronologie des messages de contrôle; corrélation avec anomalies DMX/réseau \\
RTP (générique) & Messages & Trous de séquence, gigue, désordre; base pour bilans de santé de type AES67 \\
RTP-MIDI & Messages & Timing des messages MIDI; corrélation avec signaux de conduite/codes temporels sur le réseau \\
Infrastructure réseau & Flux & Motifs IGMP/mDNS/DHCP/ARP/DNS; problèmes de multicast/découverte/adressage \\
Protocoles laser propriétaires & Flux & Perte/gigue/rafales par flux; détection multi-sources si applicable \\
\bottomrule
\end{tabularx}
\caption{Registre d’extensions non normatif (susceptible d’évoluer).}
\end{table}
...
\end{itemize}
\subsection{D?finitions des m?triques (minimal)}
\begin{itemize}
  \item \textbf{pps/bps :} paquets/octets par seconde sur une fen?tre glissante de 1.0 s.
  \item \textbf{fps (univers) :} trames DMX reconstruites par seconde sur une fen?tre glissante de 5.0 s.
  \item \textbf{gigue (IAT) :} variation moyenne entre temps d'arriv?e cons?cutifs sur une fen?tre glissante de 10.0 s (au mieux, sans interpolation).
\end{itemize}
Voir l'Appendice D pour les d?finitions op?rationnelles et les unit?s.
...
LiveShark \DOIT{} utiliser une d?finition canonique des trames DMX reconstruites (champs + 512 slots) telle que d?finie dans l'Appendice B.
\end{reqbox}

\begin{reqbox}{LS-FR-013}{P1}{}
LiveShark \DOIT{} suivre les r?gles de reconstruction des trames et de comptage du fps d?finies dans l'Appendice B.
\end{reqbox}
...
Le rapport JSON \DOIT{} ?tre d?terministe : l'ordre des listes est stable et les champs volatils sont explicitement d?finis.
\end{reqbox}

\begin{reqbox}{LS-REP-003}{P1}{}
Le rapport JSON \DOIT{} inclure les champs et types minimaux d?finis dans l'Appendice A (v0.1).
\end{reqbox}

\begin{reqbox}{LS-REP-004}{P1}{}
Les enregistrements de conflits du rapport JSON \DOIT{} suivre la structure d?finie dans l'Appendice A.
\end{reqbox}

\begin{reqbox}{LS-REP-010}{P1}{}
Le rapport JSON \DOIT{} inclure les champs minimaux d?finis dans l'Appendice A (v0.1).
\end{reqbox}

\begin{reqbox}{LS-REP-011}{P1}{}
Le rapport JSON \DOIT{} respecter les r?gles de d?terminisme d?finies dans l'Appendice A (tri stable et champs volatils).
\end{reqbox}

\begin{reqbox}{LS-REP-012}{P1}{}
Le rapport JSON \DOIT{} inclure les exemples d?crits dans l'Appendice A (minimal + conflit).
\end{reqbox}

\begin{reqbox}{LS-CONF-002}{P1}{}
LiveShark \DOIT{} utiliser la d?finition d'identit? de source pour les conflits d?finie dans l'Appendice C.
\end{reqbox}

\begin{reqbox}{LS-CONF-003}{P1}{}
LiveShark \DOIT{} calculer le chevauchement des conflits selon la d?finition de l'Appendice C.
\end{reqbox}

\begin{reqbox}{LS-CONF-010}{P1}{}
LiveShark \DOIT{} utiliser la d?finition d'identit? de source pour la d?tection des conflits d?finie dans l'Appendice C (CID + repli).
\end{reqbox}

\begin{reqbox}{LS-CONF-011}{P1}{}
LiveShark \DOIT{} calculer le chevauchement des conflits en utilisant les horodatages de capture et le seuil d?fini dans l'Appendice C (v0.1).
\end{reqbox}

\begin{reqbox}{LS-CONF-012}{P1}{}
Les enregistrements \texttt{conflicts[]} \DOIT{} inclure les champs requis d?finis dans l'Appendice C (v0.1).
\end{reqbox}

\begin{reqbox}{LS-MET-001}{P1}{}
LiveShark \DOIT{} calculer et rapporter pps/bps/fps/gigue tels que d?finis dans la Section 4.1.
\end{reqbox}

\begin{reqbox}{LS-MET-010}{P1}{}
LiveShark \DOIT{} calculer et rapporter pps/bps/fps/gigue tels que d?finis dans l'Appendice D (v0.1).
\end{reqbox}
...
\begin{itemize}
  ...
  \item \texttt{tool.name} et \texttt{tool.version} : cha?nes (identification de l'outil).
  \item \texttt{generated\_at} : chaîne, horodatage RFC3339.
  \item \texttt{input.path} : cha?ne ; \texttt{input.bytes} : entier.
  \item \texttt{capture\_summary} : objet ou null (si indisponible).
  \item \texttt{universes[]}, \texttt{flows[]}, \texttt{conflicts[]} : tableaux (peuvent ?tre vides en v0.1).
  \item \texttt{compliance[]} : tableau (peut ?tre vide en v0.1).
\end{itemize}
...
\begin{itemize}
  ...
  \texttt{compliance[]} tri?s par \texttt{protocol}, puis \texttt{violations[]} par \texttt{id}.
  \item \textbf{Champs volatils :} seuls \texttt{generated\_at} (et optionnellement \texttt{input.path}) peuvent varier.
  \item \textbf{Flottants :} aucune contrainte d'arrondi en v0.1 ; valeurs rapportées telles que calculées.
...
{
  "universe": 1,
  "sources": ["sacn:cid:11223344", "artnet:192.168.0.2:6454"],
...
\begin{itemize}
  \item Les trames sont reconstruites par univers et par source, à partir de charges utiles valides.
  \item Si une charge utile fournit moins de 512 slots, les slots manquants sont remplis par 0 pour cette trame.
  \item Les trames hors ordre sont acceptées ; la reconstruction et les métriques sont au mieux.
...
\begin{itemize}
  ...
  \item \texttt{affected\_channels[]} (liste d'entiers ; vide en v0.1)
  \item \texttt{severity} (chaîne)
  \item \texttt{conflict\_score} (flottant)
\end{itemize}
Optionnel (pr?vu, non normatif) : \texttt{first_seen}, \texttt{last_seen} et \texttt{protocols[]}.

\subsection{Appendice D --- D?finitions des m?triques (v0.1)}
\begin{itemize}
  \item \textbf{pps/bps :} paquets/octets par seconde sur une fen?tre glissante de 1.0 s ; unit?s paquets/s et octets/s.
  \item \textbf{fps (univers) :} trames DMX reconstruites par seconde sur une fen?tre glissante de 5.0 s ; unit? trames/s.
  \item \textbf{gigue (IAT) :} variation moyenne entre temps d'arriv?e cons?cutifs sur une fen?tre glissante de 10.0 s ; unit? en secondes (rapport?e en ms si applicable).
\end{itemize}
...
Les fichiers PDF sont des artefacts de compilation générés depuis les sources \texttt{.tex} et \textbf{ne doivent pas être versionnÃ©s} dans le dépôt.
