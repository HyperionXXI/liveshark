...
\begin{itemize}
  \item \textbf{Objectif :} Fournir un identifiant stable et déterministe pour chaque source afin de permettre aux fonctionnalités compare/timeline de l'IHM de suivre les sources de manière fiable entre rapports.
  \item \textbf{Format :} Suit les règles d'identité de source de l'Appendice C : \texttt{sacn:cid:<CID>}, \texttt{sacn:<ip>:<port>}, ou \texttt{artnet:<ip>:<port>}.
  ...
  \item \textbf{Guide de consommation :} Les consommateurs v0.1 peuvent ignorer ce champ. Les consommateurs v0.2 \DEVRAIT{} utiliser \texttt{source\_id} comme clé stable primaire pour les comparaisons entre rapports et le suivi d'état de l'IHM.
\end{itemize}

\subsubsection{ConflictSummary.proto (chaîne optionnelle, v0.2)}
\begin{itemize}
  \item \textbf{Objectif :} Indiquer le protocole (\texttt{artnet} ou \texttt{sacn}) des sources impliquées dans un conflit. Permet le rendu spécifique au protocole et les stratégies de résolution de conflits dans l'IHM.
  \item \textbf{Format :} Valeur de chaîne : \texttt{artnet} ou \texttt{sacn}.
  \item \textbf{Disponibilité :} Toujours complété en v0.2 pour chaque enregistrement \texttt{ConflictSummary}. Absent (null) en v0.1.
  \item \textbf{Sémantique :} Un enregistrement de conflit implique toujours des sources du même protocole ; donc \texttt{proto} est une seule chaîne, non par source.
  \item \textbf{Guide de consommation :} Les consommateurs v0.1 peuvent ignorer ce champ. Les consommateurs v0.2 \DEVRAIT{} utiliser \texttt{proto} pour étiqueter les résumés de conflits et permettre le filtrage ou le rendu conscient du protocole dans l'IHM.
\end{itemize}
