...
AES67 & Standard d'interopérabilité audio sur IP \\
...
\paragraph{Delta de séquence sACN modulo-256 (définition de base).}
Pour les flux sACN, les numéros de séquence sont des compteurs 8 bits. Soit le delta modulo 256 $\Delta = (\texttt{seq} - \texttt{seq\_prev}) \bmod 256$, où \texttt{seq\_prev} est le numéro de séquence du paquet précédent provenant de la même source. Le delta détermine la classification de l'ordre des paquets :
\begin{itemize}
  \item Si $\Delta = 0$ : paquet dupliqué (même séquence que le précédent).
  \item Si $\Delta = 1$ : paquet dans l'ordre.
  \item Si $\Delta \in [2, 127]$ : indique un trou de séquence ; le nombre de paquets manquants est $(\Delta - 1)$.
  \item Si $\Delta \in [128, 255]$ (demi-plage arrière) : paquet réordonné ; \textbf{ne} contribue \textbf{pas} à la perte.
  \item Wrap-around (ex. $\ldots, 254, 255, 0, 1, \ldots$) : traité comme dans l'ordre.
\end{itemize}

\subsection{Définitions des métriques (minimal)}
\begin{itemize}
  \item \textbf{pps/bps :} paquets/octets par seconde en moyenne sur l'intervalle actif du flux $[t_{first}, t_{last}]$. Unités paquets/s et octets/s. Omettre si moins de deux paquets horodatés existent, si $t_{last} == t_{first}$, ou si la durée active est indisponible. (Rapporté dans \texttt{flows[]}).
  \item \textbf{fps (univers) :} trames DMX reconstruites par seconde sur une fenêtre glissante de 5.0 s ; unité trames/s. La valeur rapportée est le débit sur la dernière fenêtre de 5.0 s se terminant au dernier horodatage observé.
  \item \textbf{gigue IAT (diagnostic) :} moyenne des variations absolues entre temps d'arrivée successifs sur une fenêtre glissante de 10.0 s : $\text{mean}(|IAT_i - IAT_{i-1}|)$. L'unité est la milliseconde pour \texttt{iat\_jitter\_ms} et \texttt{jitter\_ms}. La valeur rapportée est le pic (maximum) sur l'ensemble des fenêtres.
  \item \textbf{perte (observée) :} lorsque des numéros de séquence existent (v0.1 : sACN), la perte est rapportée comme des trous observés au point de capture. Pour sACN, les trous sont calculés par source en utilisant la règle du delta modulo-256 ci-dessus ; seul $\Delta \in [2,127]$ contribue à la perte observée comme $(\Delta-1)$ paquets manquants. En v0.1, la perte \DOIT{} être rapportée uniquement lorsque des numéros de séquence existent ; sinon, les champs de perte sont omis. Cela ne prouve pas une perte côté récepteur.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{pps\_peak\_1s / bps\_peak\_1s (flux) :} par flux UDP, pics de charge (paquets/s et octets/s) sur fenêtre \emph{glissante} de 1.0 s. Règle : inclure les paquets dont l'horodatage est dans $[t-1.0, t]$ (bornes incluses). Reporter le maximum des comptes de fenêtre en entier. Omettre si le flux a moins de deux paquets horodatés ou si la durée couverte est $< 1.0$ s.
    \item \textbf{dup\_packets / reordered\_packets (univers sACN) :} par univers, les compteurs sont calculés \textbf{par source sACN (CID)} en ordre temporel de capture, puis agrégés. Pour chaque source, on définit le delta modulo 256 $\Delta = (\texttt{seq} - \texttt{seq\_prev}) \bmod 256$. Si $\Delta = 0$, incrémenter \texttt{dup\_packets}. Si $\Delta = 1$, le paquet est dans l'ordre. Si $\Delta \in [2,127]$, cela indique des paquets manquants et contribue $(\Delta - 1)$ à la perte observée. Si $\Delta \in [128,255]$ (demi-plage arrière), incrémenter \texttt{reordered\_packets} ; ces deltas \textbf{ne} contribuent \textbf{pas} à la perte. Le wrap-around (ex. 254, 255, 0, 1) est dans l'ordre. Omettre ces champs si aucun numéro de séquence n'est disponible.
\end{itemize}
...
En v0.1, la perte est rapportée uniquement lorsque des numéros de séquence existent (ex. sACN).
Aucune perte n'est inférée pour Art-Net.
...
\begin{itemize}
  \item \textbf{Pourcentage de conformité :} c'est un placeholder (rapporté à 100.0 en v0.1) et \NEDOITPAS{} être interprété comme un score réel. Les consommateurs \DOIT{} utiliser \texttt{violations[]} comme source de vérité.
...
\section{Portée prévue (v0.2, non normative)}
Cette section résume l'orientation v0.2. Elle ne modifie pas le contrat v0.1.
\begin{itemize}
  \item \textbf{Timeline viewer :} navigation temporelle pour univers, flux et conflits, uniquement sur champs explicites du rapport.
  \item \textbf{Vue compare :} diff A/B déterministe en utilisant l'ordre et les clés canoniques de l'Appendice A ; l'absence est traitée comme \texttt{N/A}, pas comme zéro.
  \item \textbf{Performance viewer :} rester réactif sur les rapports volumineux (table + détails).
  \item \textbf{Hors ligne uniquement :} analyse basée sur fichiers, pas de capture live native.
  \item \textbf{Stabilité du schéma :} champs additifs optionnels, sans rupture pour les consommateurs v0.1.
\end{itemize}

\section{Portée prévue (v0.3+, non normative)}
\begin{itemize}
  \item \textbf{sACN avancé :} exposer sync, discovery et priority (y compris per-address) sans inférer de causes racines.
  \item \textbf{Symptômes réseau :} remonter des motifs observables multicast/IGMP/rafales sans attribuer la faute à un équipement.
  \item \textbf{Présence sécurité :} signaler ssACN si détecté (présence uniquement, pas d'inférence de menace).
  \item \textbf{Visibilité RDMnet :} détecter la présence et les participants (visibilité seule, pas de contrôle).
\end{itemize}
...
\begin{itemize}
  \item \textbf{Bornes de temps :} la timeline \DOIT{} utiliser les bornes (RFC3339).
  \texttt{capture\_summary.\allowbreak time\_start} et \texttt{capture\_summary.\allowbreak time\_end}.
  ...
  \item \textbf{Lignes d'univers :} une ligne d'univers est identifiée par la clé canonique \texttt{(universe,\allowbreak proto)} (Appendice A).
  Elle \DOIT{} être sélectionnable et reliée au même univers dans les vues table/détails.
  ...
  \item \textbf{Conflits sur la timeline (champs v0.2) :} les intervalles de conflits \DEVRAIT{} être représentés lorsque les champs additifs \texttt{conflicts[].\allowbreak first\_seen} et \texttt{conflicts[].\allowbreak last\_seen} (RFC3339) sont présents.
  S'ils sont absents, les conflits \DOIT{} rester visibles dans la liste/détails mais \NEDOITPAS{} être placés sur la timeline.
  \item \textbf{Pas d'inférence :} la timeline \DOIT{} visualiser uniquement les champs mesurés/\allowbreak explicites du rapport. Elle \NEDOITPAS{} créer de nouveaux événements, et \NEDOITPAS{} revendiquer des causes racines.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Clés et ordre :} les comparaisons \DOIT{} utiliser des clés canoniques dérivées des règles de l'Appendice A :
  \begin{itemize}
    \item \texttt{universes} par \texttt{(universe, proto)}.
    \item \texttt{flows} par \texttt{(src,\allowbreak dst,\allowbreak app\_proto)}. Utiliser les chaînes du rapport \textbf{telles quelles}. \textbf{Aucune normalisation} de casse, IPv6, formatage, etc.
  ...
  \end{itemize}
  \item \textbf{Absent vs zéro :} les champs numériques optionnels absents en A ou B \DOIT{} être traités comme \textbf{inconnus} (afficher \texttt{N/A}) et \NEDOITPAS{} être remplacés par zéro pour le calcul des deltas.
  \item \textbf{Égalité N/A :} si un champ est \texttt{N/A} dans \textbf{A et B}, il \NEDOITPAS{} être rapporté comme \enquote{changé}.
  \item \textbf{Présentation :} le viewer \DOIT{} classer les changements en \enquote{ajouté}, \enquote{retiré} ou \enquote{modifié}. Pour les valeurs numériques présentes dans A et B, il \PEUT{} afficher \texttt{B-A} en plus de \texttt{A \textrightarrow B}.
\end{itemize}

\subsection{Invariants et stabilité UX}
\begin{itemize}
  \item \textbf{Ordre par défaut :} sans tri utilisateur, les listes \DOIT{} apparaître dans l'ordre canonique déterministe (Appendice A).
  ...
  \item \textbf{Troncature sûre :} la troncature \PEUT{} être appliquée pour la lisibilité, mais la valeur complète \DOIT{} rester accessible (infobulle et/ou détails).
  \item \textbf{Inconnus explicites :} les champs optionnels manquants \DOIT{} être affichés comme \texttt{N/A} (jamais silencieusement comme 0).
\end{itemize}
...
\begin{enumerate}
  \item \texttt{liveshark pcap analyze --report out/report.json}\\
  \texttt{file.pcapng} fonctionne sur 3 captures. La CLI accepte aussi \texttt{analyse} comme alias.
...
\subsection{Checklist de validation (contrat de rapport)}
\begin{enumerate}
  \item Le rapport contient \texttt{report\_version}, \texttt{tool}, \texttt{generated\_at}, \texttt{input}, et les sections\\
  \texttt{capture\_summary}, \texttt{universes[]}, \texttt{flows[]}, \texttt{conflicts[]}, \texttt{compliance[]}.
  \item Les règles d'ordre déterministe sont appliquées (Appendice A).
  \item Au moins 4 tests golden : \texttt{artnet}, \texttt{artnet\_conflict}, \texttt{sacn}, \texttt{sacn\_conflict}.
  \item Les champs volatils (\texttt{generated\_at}, et optionnellement \texttt{input.path}) sont normalisés dans les tests golden.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \texttt{capture\_summary} : objet ou null (si indisponible). S'il est présent, il \DOIT{} inclure\\
  \texttt{packets\_total} (entier), et \texttt{time\_start}/\texttt{time\_end} en RFC3339 UTC avec suffixe \texttt{Z} si connus.
  ...
  v0.2 ajoute le champ optionnel \texttt{source\_id}, voir Appendice E),\\
  ...
  \item Les éléments de \texttt{conflicts[]} contiennent : \texttt{universe} (entier), \texttt{sources[]} (tableau d'identifiants de source, voir Appendice C), \texttt{proto} (chaîne ; additive v0.2, indique le protocole), \texttt{overlap\_duration\_s} (flottant), \texttt{affected\_channels[]} (tableau d'entiers ou vide), \texttt{severity} (chaîne), et \texttt{conflict\_score} (flottant). Définition complète dans l'Appendice C.
  \item Les éléments de \texttt{compliance[]} contiennent : \texttt{protocol} (chaîne),\\
  \texttt{compliance\_percentage}\\
  (flottant ; v0.1 émet 100.0 comme placeholder ; les consommateurs \NEDOITPAS{} s'y fier et \DOIT{} utiliser \texttt{violations[]} à la place),\\
  ...
  La liste d'exemples, lorsqu'elle est présente, \DOIT{} être dédupliquée, triée lexicographiquement (octet par octet) et limitée à 3 pour garder des rapports compacts et déterministes. Les exemples sont illustratifs et n'affectent pas \texttt{count}.
\end{itemize}

\paragraph{Conventions de formatage des chaînes (v0.1).}
Sauf mention contraire :
\begin{itemize}
  \item \texttt{source\_ip} est une adresse IP en forme texte (IPv4 en décimal pointé ; IPv6 en forme canonique compressée en minuscules, type RFC5952).
  \item \texttt{src} et \texttt{dst} dans \texttt{flows[]} sont des adresses socket formatées en \texttt{ip:port} pour IPv4 et \texttt{[ip]:port} pour IPv6 (forme d'affichage classique \texttt{SocketAddr}).
  \item Les consommateurs \DOIT{} comparer ces champs comme des chaînes telles qu'émises (sans normalisation supplémentaire).
\end{itemize}
...
\begin{itemize}
  ...
  \texttt{sources[]} dans chaque univers triés par \texttt{source\_id} croissant si présent (v0.2) ; pour la compatibilité v0.1, trier par \texttt{source\_ip} croissant, puis \texttt{cid} croissant (si présent ; \texttt{cid} absent en dernier) ;
  \texttt{flows[]} triés par \texttt{src} puis \texttt{dst}, puis \texttt{app\_proto} ;
  ...
  \texttt{compliance[]} triés par \texttt{protocol}, puis \texttt{violations[]} par sévérité (\texttt{error} avant \texttt{warning}), puis \texttt{id}.
  ...
  \item \textbf{RFC3339 :} \texttt{generated\_at}, \texttt{time\_start} et \texttt{time\_end} utilisent RFC3339 en UTC avec suffixe \texttt{Z}.
\end{itemize}
...
\begin{itemize}
  ...
  \texttt{loss\_rate} est \texttt{loss\_packets / (loss\_packets + seq\_tracked\_frames)}\\
  où \texttt{seq\_tracked\_frames} est le nombre de trames observées avec numéros de séquence dans la même fenêtre de 10.0 s. Si les numéros de séquence sont indisponibles, les champs de perte \DOIT{} être omis.
...
\begin{itemize}
  ...
  \item \textbf{pps\_peak\_1s / bps\_peak\_1s :} par flux, pics de charge (paquets/s et octets/s) sur une fenêtre glissante de 1.0 s, avec règle d'inclusion $[t-1.0, t]$. Reporter le maximum des comptes de fenêtre en entier. Omettre si moins de deux paquets horodatés existent ou si la durée couverte est $< 1.0$ s.
    \item \textbf{dup\_packets / reordered\_packets :} par univers, les compteurs sont calculés \textbf{par source sACN (CID)} en ordre temporel de capture, puis agrégés. Pour chaque source, on définit le delta modulo 256 $\Delta = (\texttt{seq} - \texttt{seq\_prev}) \bmod 256$. Si $\Delta = 0$, incrémenter \texttt{dup\_packets}. Si $\Delta = 1$, le paquet est dans l'ordre. Si $\Delta \in [2,127]$, cela indique des paquets manquants et contribue $(\Delta - 1)$ à la perte observée. Si $\Delta \in [128,255]$ (demi-plage arrière), incrémenter \texttt{reordered\_packets} ; ces deltas \textbf{ne} contribuent \textbf{pas} à la perte. Le wrap-around (ex. 254, 255, 0, 1) est dans l'ordre. Omettre ces champs si aucun numéro de séquence n'est disponible.
\end{itemize}
...
\subsection{Appendice E --- Champs additifs du schéma v0.2}

Cet appendice documente les champs optionnels ajoutés en v0.2 du contrat du rapport JSON. Ces champs sont \textit{additifs} et n'affectent pas \texttt{report\_version} ; les consommateurs v0.1 conformes \DOIT{} tolérer leur présence et \NEDOITPAS{} les traiter comme des incompatibilités. Tous les champs v0.2 utilisent la sémantique \texttt{skip\_serializing\_if} : ils sont omis de la sortie JSON s'ils sont null ou absents, garantissant la compatibilité rétroactive avec les workflows v0.1 existants.

\subsubsection{SourceSummary.source\_id (chaîne optionnelle, v0.2)}
\begin{itemize}
  \item \textbf{Objectif :} Fournir un identifiant stable et déterministe pour chaque source afin de permettre aux fonctionnalités compare/timeline de l'IHM de suivre les sources de manière fiable entre rapports. Ce champ est opaque ; les consommateurs \DOIVENT{} le traiter comme une clé stable et ne pas décomposer sa structure.
  \item \textbf{Format (recommandé) :} Le format recommandé est informatif uniquement et \NEDOITPAS{} être utilisé pour la logique de parsing. Les consommateurs \DEVRAIT{} traiter \texttt{source\_id} comme une chaîne opaque. À titre informatif, le motif est \texttt{proto:source}, où \texttt{proto} est le nom du protocole et \texttt{source} est l'endpoint. Spécifiquement :
  \begin{itemize}
    \item \texttt{artnet:<ip>:<port>} pour Art-Net (ex. \texttt{artnet:192.168.0.1:6454}).
    \item \texttt{sacn:<ip>:<port>} pour sACN avec endpoint IP (ex. \texttt{sacn:10.0.0.1:5568}).
    \item \texttt{sacn:cid:<CID>} pour sACN avec priorité CID (ex. \texttt{sacn:cid:000102030405060708090a0b0c0d0e0f}) ; \texttt{CID} est en hexadécimal minuscule, 32 caractères, sans séparateurs.
    \item Les endpoints IPv6 utilisent la forme canonique comprimée, entre crochets : \texttt{artnet:[2001:db8::1]:6454}, \texttt{sacn:[fe80::1]:5568}.
  \end{itemize}
  \item \textbf{Stabilité :} Dans un seul rapport, \texttt{source\_id} \DOIT{} être stable et déterministe pour une source donnée (même identité de source \ensuremath{\Rightarrow} même identifiant, tri déterministe). Entre les rapports, \texttt{source\_id} \DEVRAIT{} rester stable quand l'identité sous-jacente reste stable (p. ex., même CID sACN ou même endpoint IP:port).
  \item \textbf{Disponibilité :} Toujours complété en v0.2 quand un \texttt{SourceSummary} est émis. Absent (null) en v0.1.
  \item \textbf{Impact sur l'ordre :} Si présent en v0.2, \texttt{sources[]} sont triés par \texttt{source\_id} croissant (remplaçant la règle v0.1 de tri par \texttt{source\_ip}, puis \texttt{cid}).
  \item \textbf{Guide de consommation :} Les consommateurs v0.1 peuvent ignorer ce champ. Les consommateurs v0.2 \DEVRAIT{} utiliser \texttt{source\_id} comme clé stable primaire pour les comparaisons entre rapports et le suivi d'état de l'IHM ; il est plus fiable que \texttt{source\_ip} seul.
\end{itemize}

\subsubsection{ConflictSummary.proto (chaîne optionnelle, v0.2)}
\begin{itemize}
  \item \textbf{Objectif :} Indiquer le protocole des sources impliquées dans un conflit. Permet le rendu spécifique au protocole, les stratégies de résolution de conflits et l'extensibilité pour les futurs protocoles.
  \item \textbf{Format :} Nom de protocole ASCII minuscule (sensible à la casse). v0.2 émet \texttt{artnet} ou \texttt{sacn}. Les versions futures \MAY{} émettre d'autres noms de protocoles ; les consommateurs \DEVRAIT{} être prêts à rencontrer des valeurs inconnues.
  \item \textbf{Sémantique :} Un enregistrement de conflit implique toujours des sources du même protocole ; donc \texttt{proto} est une seule chaîne, non par source. Toutes les sources listées dans \texttt{sources[]} appartiennent au protocole nommé dans \texttt{proto}.
  \item \textbf{Disponibilité :} Toujours complété en v0.2 pour chaque enregistrement \texttt{ConflictSummary}. Absent (null) en v0.1.
  \item \textbf{Guide de consommation :} Les consommateurs v0.1 peuvent ignorer ce champ. Les consommateurs v0.2 \DEVRAIT{} utiliser \texttt{proto} pour étiqueter les résumés de conflits et permettre le filtrage ou le rendu conscient du protocole dans l'IHM. Les consommateurs \DOIT{} traiter les valeurs \texttt{proto} inconnues comme des chaînes opaques et les afficher/transmettre telles quelles sans tenter de les interpréter.
\end{itemize}
