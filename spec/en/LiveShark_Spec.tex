...
\begin{itemize}
  \item \textbf{pps/bps:} packets/bytes per second averaged over the flow active interval $[t_{first}, t_{last}]$. Units are packets/s and bytes/s. Omit if fewer than two timestamped packets exist or the active duration is unavailable. (Reported in \texttt{flows[]}.)
...
\begin{itemize}
  \item \textbf{Viewer timeline:} time-based navigation for universes, flows, and conflicts (read-only).
  \item \textbf{Compare mode:} side-by-side diff of two reports to highlight deltas in metrics and conflicts.
  \item \textbf{Performance focus:} viewer remains responsive on large reports (table and details).
...
\begin{itemize}
  ...
  \item \textbf{Presentation:} the viewer \MUST{} classify changes as \enquote{added}, \enquote{removed}, or \enquote{changed}. For numeric values present in both A and B, it \MAY{} display \texttt{B-A} in addition to \texttt{A \rightarrow B}.
\end{itemize}
