...
\begin{itemize}
  ...
  \texttt{message} (human-readable explanation), \texttt{count} (integer, number of occurrences), and optional
  \texttt{examples[]} (array of at most 3 strings, each containing concise context such as \texttt{"source IP:port @ timestamp"}).
  When present, examples \MUST{} be deduplicated, sorted stably, and limited to 3 to keep reports compact and deterministic).
\end{itemize}
...
(0..32767). For sACN, it is the big-endian \texttt{universe} field from the framing layer. The report uses this
