...
\begin{itemize}
  \item \textbf{pps/bps:} packets/bytes per second over a 1.0 s sliding window. The reported value is the peak (maximum) over all windows; units are packets/s and bytes/s. If windowed sampling is unavailable, fields are omitted. (Reported in \texttt{flows[]}.)
  \item \textbf{fps (universe):} reconstructed DMX frames per second over a 5.0 s sliding window; unit is frames/s.
  \item \textbf{jitter (IAT):} mean absolute variation between consecutive inter-arrival times over a 10.0 s sliding window: $\text{mean}(|IAT_i - IAT_{i-1}|)$. Unit is milliseconds for \texttt{iat\_jitter\_ms} and \texttt{jitter\_ms}. The reported value is the peak (maximum) over all windows.
  \item \textbf{loss (observed):} when reliable protocol sequence numbers exist (v0.1: sACN), loss is reported as observed sequence gaps at the capture point. In v0.1, loss \MUST{} be reported only when sequence numbers exist; otherwise loss fields are omitted. This does not guarantee loss at the receiver.
\end{itemize}
See Appendix D for operational definitions and units.

\paragraph{v0.2 additional diagnostic metrics}
These fields are optional additions in report schema v0.2; v0.1 consumers remain compatible by ignoring unknown fields.
\begin{itemize}
  ...
  \item \textbf{dup\_packets / reordered\_packets (sACN universes):} per universe, counts aggregated across sACN sources with valid sequence numbers. In capture-time order, \texttt{dup\_packets} increments when $\texttt{seq} = \texttt{seq\_prev}$. \texttt{reordered\_packets} increments when the modulo-256 delta $\Delta = (\texttt{seq} - \texttt{seq\_prev}) \bmod 256$ is in $[128, 255]$ (backward half-range). Wrap-around (e.g., 254, 255, 0, 1) is not considered reordering. Omit these fields when no sequence numbers are available.
\end{itemize}
...
\begin{itemize}
  \item \texttt{report\_version}: integer (schema version; v0.1 = 1, v0.2 = 2).
  ...
  \texttt{sources[]} (array of objects with \texttt{source\_ip}, optional \texttt{cid} and \texttt{source\_name};
    ...
    \texttt{dup\_packets}, \texttt{reordered\_packets} (v0.2; omitted when unavailable)
...
\begin{itemize}
  \item \textbf{pps/bps:} packets/bytes per second over a 1.0 s sliding window; units are packets/s and bytes/s. The reported value is the peak (maximum) over all windows. If windowed samples are unavailable, values are omitted. (Reported in \texttt{flows[]}.)
  \item \textbf{fps (universe):} reconstructed DMX frames per second over a 5.0 s sliding window; unit is frames/s.
  \item \textbf{jitter (IAT):} mean absolute variation between consecutive inter-arrival times over a 10.0 s sliding window: $\text{mean}(|IAT_i - IAT_{i-1}|)$. Unit is milliseconds for \texttt{iat\_jitter\_ms} and \texttt{jitter\_ms}. The reported value is the peak (maximum) over all windows.
  \item \textbf{loss (observed):} for sources with reliable sequence numbers (v0.1: sACN), loss is the sum of observed sequence gaps over a 10.0 s sliding window;\\
...
\subsection{Appendix D.1 --- v0.2 diagnostic metrics}
\begin{itemize}
  ...
  \item \textbf{dup\_packets / reordered\_packets:} per universe, counts aggregated across sACN sources with sequence numbers. \texttt{dup\_packets} increments when $\texttt{seq} = \texttt{seq\_prev}$. \texttt{reordered\_packets} increments when modulo-256 delta $\Delta = (\texttt{seq} - \texttt{seq\_prev}) \bmod 256$ is in $[128, 255]$. Wrap-around (e.g., 254, 255, 0, 1) is in-order. Omit when no sequence numbers are available.
\end{itemize}
