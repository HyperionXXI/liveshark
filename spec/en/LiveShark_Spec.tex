...
file as it grows, without requiring native capture on day one. The tool aims to surface \emph{probable causes} based on
observable indicators (loss patterns, jitter, bursts) on a best-effort basis. Precise loss localization can require multiple
capture points (for example, before and after a wireless segment).
...
\toprule
\textbf{Extension (candidate)} & \textbf{Level} & \textbf{Diagnostic value (examples)} \\
\midrule
OSC & Message & Control-message timeline; correlation with DMX/network anomalies \\
RTP (generic) & Message & Sequence gaps, jitter, reordering; basis for AES67-style health checks \\
RTP-MIDI & Message & MIDI message timing; correlation with cues/timecode over the network \\
Network infrastructure & Flow & IGMP/mDNS/DHCP/ARP/DNS patterns; multicast/discovery/addressing issues \\
Laser proprietary protocols & Flow & Loss/jitter/bursts per flow; multi-source detection when applicable \\
\bottomrule
\end{tabularx}
\caption{Non-normative extension register (subject to change).}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{jitter (IAT):} inter-arrival time jitter computed as the average variation between consecutive inter-arrival times over a 10.0 s sliding window (best-effort, no interpolation).
\end{itemize}
...
Conflict records in the JSON report \MUST{} follow the structure defined in Appendix A.
\end{reqbox}
...
\begin{itemize}
  ...
  \item Out-of-order frames are accepted; reconstruction and metrics are best-effort without panic.
