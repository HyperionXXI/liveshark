...
\appendix
\section{Appendix A --- JSON report contract (v0.1)}
\subsection{Minimal schema (fields and types)}
\begin{itemize}
  \item \texttt{report\_version}: integer (schema version).
  \item \texttt{tool}: object \{\texttt{name}: string, \texttt{version}: string\}.
  \item \texttt{generated\_at}: string, RFC3339 timestamp.
  \item \texttt{input}: object \{\texttt{path}: string, \texttt{bytes}: integer\}.
  \item \texttt{capture\_summary}: object or null (when unavailable).
  \item \texttt{universes[]}, \texttt{flows[]}, \texttt{conflicts[]}, \texttt{compliance[]}: arrays (may be empty in v0.1).
\end{itemize}
...
\subsection{Example (conflict record)}
\begin{verbatim}
{
  "universe": 1,
...
\section{Appendix B --- DMX frame reconstruction contract (v0.1)}
\subsection{Canonical DMX frame definition}
...
\subsection{Minimal reconstruction rules}
\begin{itemize}
  ...
  \item Out-of-order frames are accepted; reconstruction and metrics are best-effort.
  \item FPS counts frames that pass protocol validation and are assigned to a universe.
\end{itemize}

\section{Appendix C --- Conflict detection contract (v0.1)}
...
\subsection{Conflict record fields}
\begin{itemize}
