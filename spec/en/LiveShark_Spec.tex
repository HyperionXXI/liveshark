...
\subsection{Appendix D --- v0.2 additive schema fields}
...
\begin{itemize}
  \item \textbf{Purpose:} Provide a stable, deterministic identifier for each source to enable GUI compare/timeline features that require reliable cross-report source tracking.
  \item \textbf{Format:} Follows Appendix C source identity rules: \texttt{sacn:cid:<CID>}, \texttt{sacn:<ip>:<port>}, or \texttt{artnet:<ip>:<port>}.
  ...
  \item \textbf{Consumer guidance:} v0.1 consumers may ignore this field. v0.2 consumers \SHOULD{} use \texttt{source\_id} as the primary stable key for cross-report comparisons and GUI state tracking.
\end{itemize}

\subsubsection{ConflictSummary.proto (optional string, v0.2)}
\begin{itemize}
  \item \textbf{Purpose:} Indicate the protocol (\texttt{artnet} or \texttt{sacn}) of the sources involved in a conflict. Enables protocol-specific GUI rendering and conflict resolution strategies.
  \item \textbf{Format:} String value: \texttt{artnet} or \texttt{sacn}.
  \item \textbf{Availability:} Always populated in v0.2 for each \texttt{ConflictSummary} record. Absent (null) in v0.1.
  \item \textbf{Semantics:} A conflict record always involves sources from the same protocol; hence \texttt{proto} is a single string, not per-source.
  \item \textbf{Consumer guidance:} v0.1 consumers may ignore this field. v0.2 consumers \SHOULD{} use \texttt{proto} to tag conflict summaries and enable protocol-aware filtering or rendering in the GUI.
\end{itemize}
