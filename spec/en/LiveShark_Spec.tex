...
\begin{itemize}
  ...
  \item \textbf{RTP (generic) (Message):} sequence gaps, jitter, reordering; basis for AES67-style health checks.
  \item \textbf{RTP-MIDI (Message):} MIDI message timing; correlation with cues/timecode over the network.
  \item \textbf{Network infrastructure (Flow):} IGMP/mDNS/DHCP/ARP/DNS patterns; multicast/discovery/addressing issues.
  \item \textbf{Laser proprietary protocols (Flow):} loss/jitter/bursts per flow; multi-source detection when applicable.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{jitter (IAT):} inter-arrival time jitter computed as the average variation between consecutive inter-arrival times over a 10.0 s sliding window (heuristic, no interpolation).
  \item \textbf{loss (observed):} when protocol sequence numbers exist (e.g., sACN), loss is reported as observed sequence gaps at the capture point; otherwise it is a heuristic based on cadence gaps. This does not guarantee loss at the receiver.
\end{itemize}
...
The JSON report \MUST{} include the minimal schema fields defined in Appendix A (v0.1).
\end{reqbox}

\begin{reqbox}{LS-REP-011}{P1}{}
The JSON report \MUST{} follow the determinism rules defined in Appendix A (stable ordering and volatile fields).
\end{reqbox}

\begin{reqbox}{LS-REP-012}{P1}{}
The JSON report \MUST{} include the examples described in Appendix A (minimal + conflict).
\end{reqbox}
...
LiveShark \MUST{} compute and report pps/bps/fps/jitter as defined in Section 4.1.
\end{reqbox}
...
\begin{itemize}
  ...
  \item \textbf{Conflict detection} uses time overlap only and may produce false positives when sources are active on different channels.
...
\begin{enumerate}
  \item \texttt{liveshark pcap analyse --report out/report.json file.pcapng} runs successfully on 3 captures.
...
\begin{itemize}
  ...
  and \texttt{violations[]} (array of objects with \texttt{id}, \texttt{severity}, \texttt{message}, \texttt{count}, and optional
  \texttt{examples[]} with at most 3 strings).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Floats:} no rounding requirement in v0.1; values are reported as computed.
...
The canonical universe is the 16-bit integer decoded from the protocol payload. For Art-Net, it is the little-endian
\texttt{universe} field from ArtDMX, where \texttt{universe\_id = 256 * net + subuni} (0..32767). For sACN, it is the
big-endian \texttt{universe} field from the framing layer. The report uses this canonical value; the same universe
means identical numeric values across protocols.

\subsubsection{Minimal reconstruction rules}
\begin{itemize}
  \item Frames are reconstructed per universe and per source, using validated protocol payloads.
\item Before the first observed frame for a source and universe, slot values default to 0.
\item If a payload provides fewer than 512 slots, missing slots retain the last known value for that source and universe.
...
In v0.1, conflict detection is a conservative approximation based on time overlap; false positives are possible when
sources do not actively compete on the same channels at the same time.
...
\begin{itemize}
  ...
  \item \texttt{affected\_channels[]} (array of integers; empty in v0.1)
...
\begin{itemize}
  \item \textbf{pps/bps:} packets/bytes per second over a 1.0 s sliding window; units are packets/s and bytes/s.
  \item \textbf{fps (universe):} reconstructed DMX frames per second over a 5.0 s sliding window; unit is frames/s.
  \item \textbf{jitter (IAT):} average variation between consecutive inter-arrival times over a 10.0 s sliding window; unit is seconds (reported as ms where applicable).
  \item \textbf{loss (observed):} for sources with sequence numbers, loss is the sum of observed sequence gaps in the sliding window;
  \texttt{loss\_rate} is \texttt{loss\_packets / (loss\_packets + frames\_count)} for sequence-tracked frames.
  \item \textbf{bursts:} \texttt{burst\_count} is the number of loss bursts in the sliding window; \texttt{max\_burst\_len} is the
